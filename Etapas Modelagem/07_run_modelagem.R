...
if (nrow(especies_pendentes) == 0) {
  ...
  parametros_execucao <- data.frame(
    parametro = c(
      ...
      'modo_execucao','especie_unica','especie_partida'
    ),
    valor = c(
      lote_tamanho, pausa_minutos, max_tentativas, n_cores_inicial, n_cores, safe_mode, limiar_vif, n_replicacoes, test_percent,
      background_min, background_max, paste(metodos_modelagem, collapse = ','),
      modo_execucao, ifelse(is.null(especie_unica), '', as.character(especie_unica)), especie_partida
    ),
  ...
  for (i in 1:n_lotes) {
    ...
    for (j in idx_lote) {
      ...
      if (safe_mode && !is.null(resultado) && resultado$status != "sucesso" && is_memory_error(resultado$erro)) {
        novo_n_cores <- floor(n_cores_atual / 2)
        if (novo_n_cores < 1) novo_n_cores <- 1

        if (novo_n_cores < n_cores_atual) {
          cat("\n⚠️ Safe mode automático ativado (erro de memória). Reduzindo n_cores para ", novo_n_cores, " nas próximas espécies.\n", sep = "")
          n_cores_atual <- novo_n_cores
        } else if (n_cores_atual > 1) {
          cat("\n⚠️ Novo erro de memória. Reduzindo n_cores para 1 nas próximas espécies.\n")
          n_cores_atual <- 1
        }
      }
